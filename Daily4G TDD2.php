...
<html>
<head>
<title>Daily-4G-TDD-final</title>
<style> 
...
if (isset($_POST['send'])) {
    ...
    $nur_4g_T = "SELECT ((SELECT SUM(Indispo_sys_F_group_DI_min) FROM 4g_prs  where Cell_Name IN (SELECT Cell_Name FROM 4g_filtre_fdd)) +
    (SELECT SUM(Indispo_sys_F_group_DI_min) FROM 4g_prs WHERE Cell_Name IN (SELECT Cell_Name FROM 4g_filtre_tdd))) * pow(10, 5) /
    ((SELECT COUNT(Cell_Name) FROM 4g_prs) * 60 * 24 * $days) AS di_4g_avg";
...
$NUR_WORKS_4G = "SELECT ((SELECT SUM(Indispo_works_DI_min) from 4g_prs WHERE Cell_Name IN (SELECT Cell_Name from 4g_filtre_tdd)) * pow(10, 5) )
   / 
((SELECT COUNT(Cell_Name) FROM 4G_filtre_tdd) * 60 * 24 * " . $days . ") AS di_4g_avg_works_tdd FROM 4g_prs";
...
$di_4g_avg_works_tdd = $row['di_4g_avg_works_tdd'];

echo 'NUR-WORKS-4G: ' . $di_4g_avg_works_tdd;

$update_query = "UPDATE 4g_prs SET di_4g_avg_works_tdd = $di_4g_avg_works_tdd";
mysqli_query($cnx, $update_query);
...
AND Cell_Name IN (SELECT Cell_Name FROM 4g_filtre_tdd)";
...
mysqli_query($cnx, "TRUNCATE TABLE 4g_filtre_tdd");

$query1 = "SELECT * FROM 4G_U2020 WHERE Activation_Status = 'Active' and SUBSTR(Cell_Name, 1, 8) IN 
             (SELECT Sites FROM ssvok_test) and RAT='CELL_TDD' ";
...
$sql_create_table = "CREATE TABLE IF NOT EXISTS 4g_filtre_tdd(
...
$sql_insert = "INSERT INTO 4g_filtre_tdd ( Cell_Name, Activation_Status) 
...
$query2 = "SELECT * from 4g_prs WHERE  Cell_Name IN (SELECT Cell_Name from 4g_filtre_tdd )";
...
while ($ligne = mysqli_fetch_array($result2)) {
    ...
    <tr>
      ...
      <td><?php echo $ligne['di_4g_avg_works_tdd'] ?></td>
    </tr>
  <?php
      }
     }
